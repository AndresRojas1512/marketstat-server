...
public class DimEducationRepository : BaseRepository, IDimEducationRepository
{
    ...
    public async Task AddEducationAsync(DimEducation education)
    {
        ...
        await _dbContext.DimEducations.AddAsync(dbModel).ConfigureAwait(false);
        try
        {
    ...
    {
        var dbEducation = await _dbContext.DimEducations.FindAsync(educationId).ConfigureAwait(false);
    ...
    {
        var allDbEducations = await _dbContext.DimEducations.ToListAsync().ConfigureAwait(false);
    ...
    {
        ArgumentNullException.ThrowIfNull(education);
        var dbEducation = await _dbContext.DimEducations.FindAsync(education.EducationId).ConfigureAwait(false);
        if (dbEducation is null)
        {

...
public class ReportController : ControllerBase
{
    ...
    public async Task<ActionResult<string>> ExportSalarySummary([FromBody] SalarySummaryRequestDto requestDto)
    {
        ...
        var summary = await _factSalaryService.GetSalarySummaryAsync(request).ConfigureAwait(false);
        ...
        var jsonContent = JsonSerializer.Serialize(summary, _jsonOptions);
        ...
        var url = await _storageService.UploadReportAsync(fileName, fileBytes, "application/json").ConfigureAwait(false);

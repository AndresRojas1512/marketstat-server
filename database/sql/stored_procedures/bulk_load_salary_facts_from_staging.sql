...
-- Adjusted signature to include OUT parameters
...
CREATE OR REPLACE PROCEDURE marketstat.bulk_load_salary_facts_from_staging(
    IN    p_source_staging_table_name     TEXT, -- Name of the caller-created table
    OUT   p_inserted_count                INT,  -- Count of successfully inserted fact_salaries
    OUT   p_skipped_count                 INT   -- Count of rows that had errors and were skipped
)
LANGUAGE plpgsql
SECURITY DEFINER -- Owned by marketstat_administrator, runs with its privileges
AS $$
DECLARE
    -- Local v_inserted_fact_count and v_skipped_row_count are no longer needed,
    -- we will use the OUT parameters p_inserted_count and p_skipped_count directly.
    v_processing_staging_table_name TEXT := 'processing_facts_temp_' || translate(gen_random_uuid()::text, '-', '');
    v_sql TEXT;
    v_updated_count INT; -- Still useful for debugging specific update steps if needed
BEGIN
    -- Initialize OUT parameters at the beginning
    p_inserted_count := 0;
    p_skipped_count := 0;

    RAISE NOTICE '[ETL PROCEDURE] Starting bulk load. Source staging table: %', p_source_staging_table_name;

    -- 1. Create the internal temporary processing table (as before)
    ...
    -- 2. Populate the internal temporary processing table (as before)
    ...
    -- 4. Dimension ID Resolution (All steps 4.1 to 4.8 as before)
    -- 4.1 Dates
    ...
    -- 4.7 Job Roles (dim_job_role) - Using your refined version
    ...
    EXECUTE format('UPDATE %I SET error_message = COALESCE(error_message, '''') || %L WHERE job_role_id IS NULL AND standard_job_role_id IS NOT NULL AND hierarchy_level_id IS NOT NULL AND job_role_title IS NOT NULL AND job_role_title <> '''' AND error_message IS NULL;', v_processing_staging_table_name, 'Job Role (dim_job_role) not resolved (check dependencies: Standard Job Role, Hierarchy Level); ');
    ...
    v_sql := format('
        ...
        FROM %I WHERE error_message IS NULL
    ...
    EXECUTE v_sql;
    GET DIAGNOSTICS p_inserted_count = ROW_COUNT; -- Assign to OUT parameter
    RAISE NOTICE '[ETL PROCEDURE] Successfully inserted % salary facts.', p_inserted_count;

    -- 6. Determine skipped rows and log to 'failed_salary_facts_load' table
    EXECUTE format('SELECT COUNT(*) FROM %I WHERE error_message IS NOT NULL;', v_processing_staging_table_name)
    INTO p_skipped_count; -- Assign to OUT parameter
    ...
    IF p_skipped_count > 0 THEN
        ...
        v_sql := format('
            INSERT INTO marketstat.failed_salary_facts_load (run_timestamp, recorded_date_text, city_name, oblast_name, employer_name, standard_job_role_title, job_role_title, hierarchy_level_name, employee_birth_date_text, employee_career_start_date_text, salary_amount, bonus_amount, error_message)
            SELECT CURRENT_TIMESTAMP, recorded_date_text, city_name, oblast_name, employer_name, standard_job_role_title, job_role_title, hierarchy_level_name, employee_birth_date_text, employee_career_start_date_text, salary_amount, bonus_amount, error_message
    ...
    -- 7. Clean up internal processing table (as before)
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        EXECUTE format('DROP TABLE IF EXISTS %I;', v_processing_staging_table_name);
        -- Set OUT parameters to indicate error if they haven't been set by normal flow
        IF p_inserted_count = 0 AND p_skipped_count = 0 THEN
            -- Attempt to count total rows from source if possible, otherwise set to error indicators
            BEGIN
                EXECUTE format('SELECT COUNT(*) FROM %I', p_source_staging_table_name) INTO p_skipped_count;
                p_inserted_count := 0; -- No inserts happened
...
-- Update ALTER PROCEDURE and GRANT to reflect the new signature
...
\echo 'Procedure marketstat.bulk_load_salary_facts_from_staging (with OUT parameters for counts) created/replaced.'

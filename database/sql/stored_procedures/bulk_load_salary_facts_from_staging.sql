...
BEGIN
    ...
    -- 1. Create the internal temporary processing table with NEW COLUMNS
    v_sql := format('
        CREATE TEMP TABLE %I (
    ...
    -- 2. Populate the internal temporary processing table with NEW COLUMNS
    ...
    -- ... (Steps 4.1 to 4.7 for Date, Location, Employer, Job Roles remain the same) ...
    -- 4.1 Dates
    ...
    EXECUTE format('UPDATE %I SET error_message = COALESCE(error_message, '''') || %L WHERE job_role_id IS NULL AND standard_job_role_id IS NOT NULL AND hierarchy_level_id IS NOT NULL AND job_role_title IS NOT NULL AND job_role_title <> '''' AND error_message IS NULL;', v_processing_staging_table_name, 'Job Role (dim_job_role) not resolved (check dependencies); ');


    -- 4.8 Employees (Get-or-Create, MODIFIED to use employee_ref_id)
    RAISE NOTICE '[ETL PROCEDURE] Processing employees...';
    EXECUTE format('
        WITH employees_to_process AS (
            SELECT DISTINCT
                ssf.employee_ref_id,
                ssf.employee_birth_date_text,
                ssf.employee_career_start_date_text,
                ssf.gender
            FROM %I ssf
            WHERE ssf.employee_id IS NULL AND ssf.error_message IS NULL
              AND ssf.employee_ref_id IS NOT NULL AND ssf.employee_ref_id <> ''''
              -- Basic validation for dates
              AND pg_catalog.pg_input_is_valid(ssf.employee_birth_date_text, ''date'')
              AND pg_catalog.pg_input_is_valid(ssf.employee_career_start_date_text, ''date'')
        ),
        upserted_employees AS (
            INSERT INTO marketstat.dim_employee (employee_ref_id, birth_date, career_start_date, gender)
            SELECT
                etp.employee_ref_id,
                CAST(etp.employee_birth_date_text AS DATE),
                CAST(etp.employee_career_start_date_text AS DATE),
                etp.gender
            FROM employees_to_process etp
            ON CONFLICT (employee_ref_id) -- Use the new unique key
            DO UPDATE SET -- If employee already exists, update their details
                birth_date = EXCLUDED.birth_date,
                career_start_date = EXCLUDED.career_start_date,
                gender = EXCLUDED.gender
            RETURNING employee_id, employee_ref_id
        )
        UPDATE %I ssf SET employee_id = ue.employee_id
        FROM upserted_employees ue
        WHERE ssf.employee_ref_id = ue.employee_ref_id
          AND ssf.employee_id IS NULL AND ssf.error_message IS NULL;',
        v_processing_staging_table_name, v_processing_staging_table_name
    );
    -- Error logging for employees
    EXECUTE format('UPDATE %I SET error_message = COALESCE(error_message, '''') || %L WHERE employee_id IS NULL AND employee_ref_id IS NOT NULL AND employee_ref_id <> '''' AND error_message IS NULL;',
                   v_processing_staging_table_name, 'Employee not resolved (check source ref_id and date formats); ');
    EXECUTE format('UPDATE %I SET error_message = COALESCE(error_message, '''') || %L WHERE employee_ref_id IS NULL OR employee_ref_id = '''';',
                   v_processing_staging_table_name, 'Missing employee_ref_id in source data; ');
...
\echo 'Procedure marketstat.bulk_load_salary_facts_from_staging (with employee_ref_id) created/replaced.'
